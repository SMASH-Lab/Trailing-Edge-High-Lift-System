...
<tr><td></td>
  ...
  function PropertyToInteger
    "Convert  Property (Violated, Undecided, Satisfied) to Integer (1,2,3)"
     ...
     input Modelica_Requirements.Types.Property u
      "Property to be mapped to Integer";
...
<tr><td></td>
  ...
  function PropertiesToProperty
    "Convert a Property vector to one Property (Violated, Undecided, Satisfied)"
    extends Modelica_Requirements.Interfaces.PropertyFunction;
    import Modelica_Requirements.Types.Property;
    import Modelica_Requirements.LogicalFunctions.cardViolated;
    import Modelica_Requirements.LogicalFunctions.cardSatisfied;

    input Property p[:] "Property vector";
    output Property result
      "= Violated, if at least one element is Violated; Satisfied if no element is violated and at least one is Satisfied; otherwise Undecided.";
  algorithm
    result :=if size(p, 1) < 1 then Property.Undecided elseif cardViolated(p) > 0
       then Property.Violated elseif cardSatisfied(p) > 0 then Property.Satisfied
...
<blockquote><p>
<b>PropertiesToProperty</b>(p);
</p></blockquote>

<h4>Description</h4>
<p>
The function returns Property.Violated, if at least <b>one element</b> of Property vector p has value Violated.<br>
The function returns Property.Satisfied, if no element of p is Violated and at least one element is Satisfied.<br>
Otherwise, the function returns Property.Undecided.
</p>
</html>"));
...
<tr><td></td>
  ...
  function PropertyToCumulatedProperty "Convert  Property to cumulated Property"
     import Modelica_Requirements.Types.Property;
     import Modelica_Requirements.Types.CumulatedProperty;
     extends Modelica.Icons.Function;

     input Property u "Property to be mapped to cumulated property";
     input CumulatedProperty pre_y "Previous value of cumulated property";
     output CumulatedProperty y "Cumulated property";
  algorithm
    y := if pre_y == CumulatedProperty.Untested then
            (if u == Property.Satisfied then CumulatedProperty.Satisfied else
             if u == Property.Violated  then CumulatedProperty.Violated else
                                             CumulatedProperty.Untested) else
         if pre_y == CumulatedProperty.Satisfied then
            (if u == Property.Violated then CumulatedProperty.Violated else
                                            CumulatedProperty.Satisfied)
         else CumulatedProperty.Violated;

    annotation (Inline=true, Documentation(info="<html>
</html>"));
  end PropertyToCumulatedProperty;
